...
# python Predict_chunk.py --fasta test_head20.fasta --model_path ICCTax.pth --output ICCTax_chunk.txt --mapping_dir ./mapping
def parse_args():
    ...
    parser.add_argument('--max_length', type=int, default=1500, help="Maximum length for sequence splitting (default: 1500)")
...
def predict_sequences(sequences, model, tokenizer, device, batch_size, max_length):
...
def main():
    ...
    supk_labels, phyl_labels, genus_labels = predict_sequences(
        sequences, model, tokenizer, device, args.batch_size, args.max_length
    )
